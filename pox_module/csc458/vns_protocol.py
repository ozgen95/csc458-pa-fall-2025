...
class VNSOpen(LTMessage):
    ...
    _FORMAT = (
        f"> HH {VNSConstants.ID_SIZE}s {VNSConstants.ID_SIZE}s {VNSConstants.ID_SIZE}s"
    )
    _SIZE = struct.calcsize(_FORMAT)

    def __init__(
        self, topology_id: int, virtual_host_id: str, user_id: str, password: str
    ) -> None:
        super().__init__()
...
class VNSPacket(LTMessage):
    ...
    def unpack(body: bytes) -> "VNSPacket":
        header_data = struct.unpack(
            VNSPacket._HEADER_FORMAT, body[: VNSPacket._HEADER_SIZE]
        )
        interface_name = NetworkAddressValidator.strip_null_characters_from_bytes(
            header_data[0]
        )
...
class VNSInterface:
    ...
    def __init__(
        self, name: str, mac_address: str, ip_address: str, subnet_mask: str
    ) -> None:
...
class VNSOpenTemplate(LTMessage):
    ...
    def pack(self) -> bytes:
        ...
        filter_data = b"".join(
            inet_aton(ip) + struct.pack(">B", mask) for ip, mask in self.source_filters
        )
    ...
    def unpack(body: bytes) -> "VNSOpenTemplate":
        ...
        template_name = NetworkAddressValidator.strip_null_characters_from_bytes(
            header_data[0]
        )
        virtual_host_id = NetworkAddressValidator.strip_null_characters_from_bytes(
            header_data[1]
...
class VNSAuthReply(LTMessage):
    ...
    def length(self) -> int:
        return (
            len(self.username) + len(self.salted_password_hash) + 4
        )  # +4 for username length
...
class VNSAuthStatus(LTMessage):
    ...
    def __str__(self) -> str:
        ...
        return (
            f"AUTH_STATUS: auth_ok={self.authentication_successful} msg={message_str}"
        )
