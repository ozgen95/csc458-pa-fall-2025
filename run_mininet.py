...
"""
Start up a Simple topology for CSC458
"""
...
class CSC458Controller(Controller):
    "Controller for CSC458 Multiple IP Bridge"
...
def stopsshd():
    ...
    info(
        "*** Shutting down stale sshd/Banner processes ",
...
def starthttp(host):
    ...
    info("*** Starting http.server on host", host, "\n")
...
def stophttp():
    ...
    info(
        "*** Shutting down stale http.server",
        quietRun("pkill -9 -f http.server"),
        "\n",
    )
    info(
        "*** Shutting down stale webservers", quietRun("pkill -9 -f webserver.py"), "\n"
    )
...
def disableIPv6(net):
    ...
    # Also disable IPv6 on the switch/router
...
def csc458net():
    stophttp()
    "Create a simple network for csc458"
    get_ip_setting()
    topo = CSC458Topo()
